...
if(isset($_GET['id'])) {
    ...
    // Sanitize the ID (example using mysqli_real_escape_string)
    $sanitized_id = mysqli_real_escape_string($con, $id);

    $q = "SELECT * FROM list_of_movies WHERE id = $sanitized_id";

    $result = mysqli_query($con, $q) or die("error: " . mysqli_error($con));
...
<body>
    ...
    <form method="post" action="edited.php" enctype="multipart/form-data">
        ...
        <select name="genre" id="genre">
            <option value="none"<?php if ($data['genre'] == 'none') echo $data['genre']; ?>>Select a genre</option>
